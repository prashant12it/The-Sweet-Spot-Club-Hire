    ...
    <title>Golf Club Courier Australia | {{ $title or 'The Sweet Spot Club Courier' }}</title>
    <!-- Bootstrap -->
    ...
    <!-- style main css file -->
    <link href="{{ URL::asset('frontend/css/clubcourier.css') }}" rel="stylesheet">
    ...
    <link rel="stylesheet" type="text/css" href="{{ URL::asset('theme/assets/skin/default_skin/css/clubcourierPrashant.css') }}">
    <!-- responsive style -->
    ...
    <script>
        ...
        var StorageImageRelPath = '{{(getenv('APP_ENV') == 'local'?'/':(getenv('APP_ENV') == 'live'?'/app/public/':'/'))}}';
    </script>
    <script src="{{ URL::asset('theme/vendor/jquery/jquery-1.11.1.min.js') }}"></script>
    <script type="text/javascript">
        var __SITE_URL__ = '{{url('/')}}';
        @if($title == 'home')
//            window.location.href = 'http://tssclubhire.com/wordpress/';
            @endif
    </script>
